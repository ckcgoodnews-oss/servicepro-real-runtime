...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`app-shell ${mobileOpen ? 'nav-open' : ''}`}>
    ...
    <p className="sr-only" role="status" aria-live="polite" aria-atomic="true">{crumbs.at(-1)?.label || 'Workspace'} page loaded</p>
    ...
    <aside className="sidebar" aria-label="Primary navigation">
      ...
      <WorkspaceHeader platformAdmin={isPlatformAdmin} />
      ...
      <div className="sidebar-bottom">{session?.user.roles?.includes('owner') && <Link href="/storefront-builder"><span aria-hidden="true">W</span>Public storefront</Link>}{session?.user.roles?.includes('owner') && <Link href="/team"><span aria-hidden="true">T</span>Team management</Link>}{isPlatformAdmin && <Link href="/platform-admin"><span aria-hidden="true">A</span>Platform admin</Link>}<Link href="/system-status"><span aria-hidden="true">●</span>System status</Link><Link href="/settings"><span aria-hidden="true">⚙</span>Settings</Link><LogoutButton /><div className="user-chip"><b>{initials}</b><span><strong>{userName}</strong><small>{session?.user.roles?.[0] || 'Workspace member'}</small></span></div></div>
    </aside>
