...
export function PlatformAdminWorkspace() {
  ...
  useEffect(() => {
    ...
    authFetch('/api/v1/admin/workspaces').then(response=>response.json()).then(body=>setWorkspaces(body.data||[])).catch(()=>setWorkspaces([]));
  }, [load]);
  ...
  async function createOwner(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await authFetch('/api/v1/platform/owners', {
      method: 'POST',
      body: JSON.stringify({tenantId: form.get('tenantId') || '', workspaceName: form.get('workspaceName'), name: form.get('name'), email: form.get('email'), password: form.get('password'), modules: form.getAll('modules'), siteTypeItemId: form.get('siteTypeItemId')})
    });
    const body = await response.json();
    setCreating(false);
  ...
  return (
    ...
    <section className="panel platform-owner-create">
      ...
      <form onSubmit={createOwner}>
        <div className="form-columns"><label>Owner name<input name="name" required /></label><label>New workspace name<input name="workspaceName" placeholder="Phillips Plumbing" /></label></div>
        <label>Existing workspace (optional)<select name="tenantId" defaultValue=""><option value="">Create a new workspace</option>{workspaces.map(workspace=><option value={workspace.tenantId} key={workspace.id}>{workspace.name} · {workspace.tenantId}</option>)}</select><small>Select an existing business or leave blank to provision a new isolated workspace.</small></label>
