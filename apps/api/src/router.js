...
const certificateLifecycle = require('./routes/certificateLifecycle');
const enterpriseSecrets = require('./routes/enterpriseSecrets');
...
async function router(req, res) {
  ...
  if(req.method==='POST'&&policyCreates[req.url]){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return policyManagement[policyCreates[req.url]](req,res);}
  ...
  if(policyAction&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;const h={'policy-versions:submit':'submitVersion','policy-versions:decide':'decideVersion','policy-versions:publish':'publishVersion','policy-acknowledgements:acknowledge':'acknowledge','policy-exceptions:decide':'decideException'}[`${policyAction[1]}:${policyAction[3]}`];if(h)return policyManagement[h](req,res,policyAction[2]);}
  if(req.url==='/api/v1/governance/policy-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.GOVERNANCE_READ)(req,res))return;return policyManagement.metrics(req,res);}
  const riskCreates={'/api/v1/governance/risks':'createRisk','/api/v1/governance/risk-kris':'createKri','/api/v1/governance/risk-treatments':'createTreatment'};if(req.method==='POST'&&riskCreates[req.url]){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return enterpriseRisk[riskCreates[req.url]](req,res)}
  const er=req.url.match(/^\/api\/v1\/governance\/(risks|risk-kris|risk-treatments)\/([^/]+)\/(assess|measure|complete)$/);if(er&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;const h={assess:'assessRisk',measure:'measureKri',complete:'completeTreatment'};return enterpriseRisk[h[er[3]]](req,res,er[2])}
  if(req.url==='/api/v1/governance/risk-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.GOVERNANCE_READ)(req,res))return;return enterpriseRisk.metrics(req,res)}
  const bcCreates={'/api/v1/governance/continuity-plans':'createPlan','/api/v1/governance/recovery-procedures':'createProcedure','/api/v1/governance/continuity-exercises':'createExercise'};if(req.method==='POST'&&bcCreates[req.url]){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return businessContinuity[bcCreates[req.url]](req,res)}
  const bc=req.url.match(/^\/api\/v1\/governance\/(continuity-plans|continuity-exercises)\/([^/]+)\/(approve|activate|start|complete)$/);if(bc&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;const h={approve:'approvePlan',activate:'activatePlan',start:'startExercise',complete:'completeExercise'};return businessContinuity[h[bc[3]]](req,res,bc[2])}
  if(req.url==='/api/v1/governance/continuity-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.GOVERNANCE_READ)(req,res))return;return businessContinuity.metrics(req,res)}
  const vendorCreates={'/api/v1/governance/vendors':'createVendor','/api/v1/governance/vendor-questionnaires':'createQuestionnaire','/api/v1/governance/vendor-documents':'createDocument','/api/v1/governance/vendor-reviews':'createReview'};if(req.method==='POST'&&vendorCreates[req.url]){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return vendorRiskManagement[vendorCreates[req.url]](req,res)}const vq=req.url.match(/^\/api\/v1\/governance\/vendor-questionnaires\/([^/]+)\/(submit|review)$/);if(vq&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return vendorRiskManagement[vq[2]==='submit'?'submitQuestionnaire':'reviewQuestionnaire'](req,res,vq[1])}if(req.url==='/api/v1/governance/vendor-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.GOVERNANCE_READ)(req,res))return;return vendorRiskManagement.metrics(req,res)}
  const assetCreates={'/api/v1/governance/assets':'createAsset','/api/v1/governance/asset-software':'createSoftware','/api/v1/governance/saas-subscriptions':'createSubscription'};if(req.method==='POST'&&assetCreates[req.url]){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return enterpriseAssetInventory[assetCreates[req.url]](req,res)}const aa=req.url.match(/^\/api\/v1\/governance\/assets\/([^/]+)\/(transfer|transition)$/);if(aa&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return enterpriseAssetInventory[aa[2]==='transfer'?'transferOwnership':'transitionLifecycle'](req,res,aa[1])}if(req.url==='/api/v1/governance/asset-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.GOVERNANCE_READ)(req,res))return;return enterpriseAssetInventory.metrics(req,res)}
  if(req.url==='/api/v1/governance/domains'&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return certificateLifecycle.createDomain(req,res)}if(req.url==='/api/v1/governance/certificates'&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return certificateLifecycle.createCertificate(req,res)}const ca=req.url.match(/^\/api\/v1\/governance\/certificates\/([^/]+)\/(activate|renew|rotate|revoke)$/);if(ca&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return certificateLifecycle[ca[2]](req,res,ca[1])}if(req.url==='/api/v1/governance/certificate-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.GOVERNANCE_READ)(req,res))return;return certificateLifecycle.metrics(req,res)}
  if(req.url==='/api/v1/governance/secrets'&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return enterpriseSecrets.create(req,res)}const sa=req.url.match(/^\/api\/v1\/governance\/secrets\/([^/]+)\/(schedule|begin|complete|revoke)$/);if(sa&&req.method==='POST'){if(!requirePermission(PERMISSIONS.GOVERNANCE_WRITE)(req,res))return;return enterpriseSecrets[sa[2]](req,res,sa[1])}if(req.url==='/api/v1/governance/secret-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.GOVERNANCE_READ)(req,res))return;return enterpriseSecrets.metrics(req,res)}
