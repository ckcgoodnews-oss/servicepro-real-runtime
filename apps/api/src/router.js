...
async function router(req, res) {
  ...
  if(req.url==='/api/v1/governance/domains'&&req.method==='POST'){if(!requirePermission(PERMISSIONS.PRIVACY_WRITE)(req,res))return;return certificateLifecycle.createDomain(req,res)}if(req.url==='/api/v1/governance/certificates'&&req.method==='POST'){if(!requirePermission(PERMISSIONS.PRIVACY_WRITE)(req,res))return;return certificateLifecycle.createCertificate(req,res)}const ca=req.url.match(/^\/api\/v1\/governance\/certificates\/([^/]+)\/(activate|renew|rotate|revoke)$/);if(ca&&req.method==='POST'){if(!requirePermission(PERMISSIONS.PRIVACY_WRITE)(req,res))return;return certificateLifecycle[ca[2]](req,res,ca[1])}if(req.url==='/api/v1/governance/certificate-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.PRIVACY_READ)(req,res))return;return certificateLifecycle.metrics(req,res)}
  if(req.url==='/api/v1/governance/secrets'&&req.method==='POST'){if(!requirePermission(PERMISSIONS.PRIVACY_WRITE)(req,res))return;return enterpriseSecrets.create(req,res)}const sa=req.url.match(/^\/api\/v1\/governance\/secrets\/([^/]+)\/(schedule|begin|complete|revoke)$/);if(sa&&req.method==='POST'){if(!requirePermission(PERMISSIONS.PRIVACY_WRITE)(req,res))return;return enterpriseSecrets[sa[2]](req,res,sa[1])}if(req.url==='/api/v1/governance/secret-metrics'&&req.method==='GET'){if(!requirePermission(PERMISSIONS.PRIVACY_READ)(req,res))return;return enterpriseSecrets.metrics(req,res)}
