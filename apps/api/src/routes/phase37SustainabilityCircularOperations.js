...
function repository(req) { return req.context.repositories.phase37SustainabilityCircularOperations; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/sustainability-circular-operations\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.SUSTAINABILITY_CIRCULAR_OPERATIONS_READ : PERMISSIONS.SUSTAINABILITY_CIRCULAR_OPERATIONS_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
