...
function repository(req) { return req.context.repositories.phase35AgenticWorkforceOrchestration; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/agentic-workforce-orchestration\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.AGENTIC_WORKFORCE_ORCHESTRATION_READ : PERMISSIONS.AGENTIC_WORKFORCE_ORCHESTRATION_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
