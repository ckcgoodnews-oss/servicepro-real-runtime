...
function repository(req) { return req.context.repositories.phase32Version5FoundationRc; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version5-foundation\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION5_FOUNDATION_READ : PERMISSIONS.VERSION5_FOUNDATION_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
