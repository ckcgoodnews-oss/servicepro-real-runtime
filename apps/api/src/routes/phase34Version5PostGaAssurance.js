...
function repository(req) { return req.context.repositories.phase34Version5PostGaAssurance; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version5-post-ga-assurance\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION5_POST_GA_ASSURANCE_READ : PERMISSIONS.VERSION5_POST_GA_ASSURANCE_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
