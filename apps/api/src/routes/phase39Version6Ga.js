...
function repository(req) { return req.context.repositories.phase39Version6Ga; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version6-ga\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION6_GA_READ : PERMISSIONS.VERSION6_GA_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
