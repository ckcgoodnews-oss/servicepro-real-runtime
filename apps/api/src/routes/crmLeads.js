...
function repo(req) { return req.context.repositories.crmLeads; }
...
function getById(req, res, id) {
  Promise.resolve(repo(req).findById(tenant(req), id))
    .then(data => data ? sendJson(res, 200, { data }) : sendJson(res, 404, { error: { code: 'not_found', message: 'Lead not found' } }));
}
...
function remove(req, res, id) {
  Promise.resolve(repo(req).delete(tenant(req), id))
    .then(data => data ? sendJson(res, 200, { data: { deleted: true } }) : sendJson(res, 404, { error: { code: 'not_found', message: 'Lead not found' } }));
}

function pipeline(req, res) {
  Promise.resolve(repo(req).pipeline(tenant(req)))
    .then(data => sendJson(res, 200, { data }));
}

module.exports = { list, getById, create, update, remove, pipeline };
