...
async function list(req,res){if(!isPlatformAdmin(req))return deny(res);return sendJson(res,200,{data:await req.context.repositories.accessEntitlements.listOwners()});}
async function issue(req,res,userId){if(!isPlatformAdmin(req))return deny(res);const days=Math.max(1,Math.min(3650,Number(req.body.days||30)));const raw=`sp_access_${issueOpaqueToken(24)}`;const row=await req.context.repositories.accessEntitlements.issue({tenantId:req.body.tenantId,userId,tokenHash:hashToken(raw),tokenLastFour:raw.slice(-4),expiresAt:new Date(Date.now()+days*86400000).toISOString(),createdBy:req.context.userId});return sendJson(res,201,{data:{...row,token:raw}});}
...
module.exports={list,issue,redeem,update};
