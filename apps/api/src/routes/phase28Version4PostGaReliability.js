...
function repository(req) { return req.context.repositories.phase28Version4PostGaReliability; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version4-post-ga\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION4_POST_GA_READ : PERMISSIONS.VERSION4_POST_GA_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
