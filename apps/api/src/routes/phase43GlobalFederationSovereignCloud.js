...
function repository(req) { return req.context.repositories.phase43GlobalFederationSovereignCloud; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/global-federation-sovereign-cloud\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.GLOBAL_FEDERATION_SOVEREIGN_CLOUD_READ : PERMISSIONS.GLOBAL_FEDERATION_SOVEREIGN_CLOUD_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
