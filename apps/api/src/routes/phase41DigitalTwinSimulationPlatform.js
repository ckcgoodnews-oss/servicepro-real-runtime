...
function repository(req) { return req.context.repositories.phase41DigitalTwinSimulationPlatform; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/digital-twin-simulation-platform\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.DIGITAL_TWIN_SIMULATION_PLATFORM_READ : PERMISSIONS.DIGITAL_TWIN_SIMULATION_PLATFORM_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
