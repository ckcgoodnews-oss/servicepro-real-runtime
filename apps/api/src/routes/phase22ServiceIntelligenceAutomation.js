...
function repository(req) { return req.context.repositories.phase22ServiceIntelligenceAutomation; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/service-intelligence-automation\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.SERVICE_INTELLIGENCE_AUTOMATION_READ : PERMISSIONS.SERVICE_INTELLIGENCE_AUTOMATION_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
