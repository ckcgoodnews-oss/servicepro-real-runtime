...
function repository(req) { return req.context.repositories.phase36SpatialRoboticsOperations; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/spatial-robotics-operations\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.SPATIAL_ROBOTICS_OPERATIONS_READ : PERMISSIONS.SPATIAL_ROBOTICS_OPERATIONS_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
