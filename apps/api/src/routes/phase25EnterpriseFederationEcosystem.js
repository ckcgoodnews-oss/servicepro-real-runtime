...
function repository(req) { return req.context.repositories.phase25EnterpriseFederationEcosystem; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/enterprise-federation-ecosystem\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.ENTERPRISE_FEDERATION_ECOSYSTEM_READ : PERMISSIONS.ENTERPRISE_FEDERATION_ECOSYSTEM_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
