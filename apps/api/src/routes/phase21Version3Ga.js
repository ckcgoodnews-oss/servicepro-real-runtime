...
function repository(req) { return req.context.repositories.phase21Version3Ga; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version3-ga\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION3_GA_READ : PERMISSIONS.VERSION3_GA_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
