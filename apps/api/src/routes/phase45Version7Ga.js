...
function repository(req) { return req.context.repositories.phase45Version7Ga; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version7-ga\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION7_GA_READ : PERMISSIONS.VERSION7_GA_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
