...
function repository(req) { return req.context.repositories.phase27Version4Ga; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version4-ga\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION4_GA_READ : PERMISSIONS.VERSION4_GA_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
