...
function repo(req) {
  return req.context.repositories.estimates;
}
...
function get(req, res, id) {
  Promise.resolve(repo(req).findById(tenant(req), id)).then(estimate => {
    if (!estimate) return sendJson(res, 404, { error: { code: 'not_found', message: 'Estimate not found' } });
...
function create(req, res) {
  Promise.resolve()
    ...
    .then(estimate => sendJson(res, 201, { data: estimate }))
...
function remove(req, res, id) {
  Promise.resolve(repo(req).delete(tenant(req), id)).then(deleted => {
    if (!deleted) return sendJson(res, 404, { error: { code: 'not_found', message: 'Estimate not found' } });
