...
function repository(req) { return req.context.repositories.phase31WorkforceCommerceNetwork; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/workforce-commerce\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.WORKFORCE_COMMERCE_READ : PERMISSIONS.WORKFORCE_COMMERCE_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
