...
function repository(req) { return req.context.repositories.phase33Version5Ga; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version5-ga\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION5_GA_READ : PERMISSIONS.VERSION5_GA_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
