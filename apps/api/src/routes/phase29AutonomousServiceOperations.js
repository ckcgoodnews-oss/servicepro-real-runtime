...
function repository(req) { return req.context.repositories.phase29AutonomousServiceOperations; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/autonomous-operations\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.AUTONOMOUS_OPERATIONS_READ : PERMISSIONS.AUTONOMOUS_OPERATIONS_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
