...
function repository(req) { return req.context.repositories.phase24FinancialGrowthOperations; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/financial-growth-operations\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.FINANCIAL_GROWTH_OPERATIONS_READ : PERMISSIONS.FINANCIAL_GROWTH_OPERATIONS_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
