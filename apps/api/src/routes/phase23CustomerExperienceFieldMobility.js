...
function repository(req) { return req.context.repositories.phase23CustomerExperienceFieldMobility; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/customer-experience-field-mobility\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.CUSTOMER_EXPERIENCE_FIELD_MOBILITY_READ : PERMISSIONS.CUSTOMER_EXPERIENCE_FIELD_MOBILITY_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
