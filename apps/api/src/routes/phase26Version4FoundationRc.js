...
function repository(req) { return req.context.repositories.phase26Version4FoundationRc; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/version4-foundation-rc\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.VERSION4_FOUNDATION_RC_READ : PERMISSIONS.VERSION4_FOUNDATION_RC_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
