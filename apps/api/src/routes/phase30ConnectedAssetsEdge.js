...
function repository(req) { return req.context.repositories.phase30ConnectedAssetsEdge; }
...
function dispatch(req,res) {
  const match=req.url.match(/\/api\/v1\/connected-edge\/([^/?]+)(?:\/([^/?]+))?(?:\/([^/?]+))?/);
  if(!match)return false;
  if(!allowed(req, req.method==='GET' ? PERMISSIONS.CONNECTED_EDGE_READ : PERMISSIONS.CONNECTED_EDGE_WRITE)){sendJson(res,403,{error:{code:'forbidden',message:'Permission denied'}});return true;}
