...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`app-shell ${mobileOpen ? 'nav-open' : ''}`}>
    ...
    <aside className="sidebar" aria-label="Primary navigation">
      ...
      <div className="sidebar-bottom"><Link href="/system-status"><span aria-hidden="true">●</span>System status</Link><Link href="/settings"><span aria-hidden="true">⚙</span>Settings</Link><LogoutButton /><div className="user-chip"><b>{initials}</b><span><strong>{userName}</strong><small>{session?.user.roles?.[0] || 'Workspace member'}</small></span></div></div>
    </aside>
