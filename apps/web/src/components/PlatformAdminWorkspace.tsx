    ...
    <div className="platform-admin-grid">
      ...
      <section className="panel platform-owner-create">
        ...
        <form onSubmit={createOwner}>
          ...
          <label>Temporary password<PasswordInput name="password" minLength={12} required /><small>Use uppercase, lowercase, number, and symbol.</small></label>
      ...
      <section className="panel">
        ...
        <div className="organization-list">
          {owners.map(owner=><article key={owner.id}>
            ...
            <details><summary>Edit owner info</summary><form onSubmit={async (event)=>{event.preventDefault();const form=new FormData(event.currentTarget);const response=await authFetch(`/api/v1/platform/owners/${owner.id}/profile`,{method:'PATCH',body:JSON.stringify({name:form.get('ownerName'),email:form.get('ownerEmail'),tenantId:owner.tenantId})});if(response.ok){setError('');await load();}else{const body=await response.json().catch(()=>({}));setError(body.error?.message||'Update failed');}}}><div className="form-columns"><label>Name<input name="ownerName" defaultValue={owner.name||''} required /></label><label>Email<input name="ownerEmail" type="email" defaultValue={owner.email||''} required /></label></div><button className="button button-small" type="submit">Save changes</button></form></details>
