...
type Dashboard = { generatedAt:string; kpis:{openWork:number;appointmentsToday:number;customers:number;outstanding:number}; attention:WorkItem[]; recentWork:WorkItem[]; notifications:{id:string;subject:string;status:string;createdAt:string}[]; activity:{id:string;eventType:string;action:string;entityType:string;displayTitle?:string;displayContext?:string;createdAt:string}[] };
...
export function DashboardOverview() {
  ...
  return <>{error && <p className="dashboard-error" role="alert">{error}</p>}<section className="kpi-grid" aria-label="Key performance indicators">{cards.map(([label,value,helper]) => <article key={label}><span>{label}</span><strong>{value}</strong><small>{helper}</small></article>)}</section>
    ...
    <div className="dashboard-grid lower-grid"><section className="panel"><div className="panel-heading"><div><h2>Recent work</h2><p>Latest tenant activity.</p></div></div><div className="compact-list">{data.recentWork.map(item => <article key={item.id}><span><strong>{item.title}</strong><small>{item.customer}</small></span><em>{item.status}</em><time>{relative(item.updatedAt)}</time></article>)}</div></section><section className="panel"><div className="panel-heading"><div><h2>Recent activity</h2><p>Customer visits, team actions, and notifications.</p></div></div><div className="compact-list">{[...data.notifications.map(item => ({id:item.id,title:item.subject,meta:item.status,at:item.createdAt})),...data.activity.map(item => ({id:item.id,title:item.displayTitle || 'ServicePro activity',meta:item.displayContext || 'Application',at:item.createdAt}))].sort((a,b)=>Date.parse(b.at)-Date.parse(a.at)).slice(0,5).map(item => <article key={`${item.id}-${item.at}`}><span><strong>{item.title}</strong><small>{item.meta}</small></span><time>{relative(item.at)}</time></article>)}</div></section></div>
  </>;
}
