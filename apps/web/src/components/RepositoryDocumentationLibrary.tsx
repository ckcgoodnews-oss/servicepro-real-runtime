...
export type DocumentationRole = 'platform_admin'|'owner'|'staff';
...
const roleCategories:Record<DocumentationRole,string[]|null>={
  platform_admin:null,
  owner:['Platform overview','Product','User guides','Operations and reference'],
  staff:['User guides'],
};

const roleLabels:Record<DocumentationRole,string>={platform_admin:'platform administrator',owner:'business owner',staff:'staff or technician'};
...
export function RepositoryDocumentationLibrary({role}:{role:DocumentationRole}){
  ...
  const permittedDocuments=useMemo(()=>{if(!library)return[];const categories=roleCategories[role];return categories?library.documents.filter(document=>categories.includes(document.category)):library.documents;},[library,role]);
  const permittedCategories=useMemo(()=>{if(!library)return[];const categories=roleCategories[role];return Object.entries(library.categories).filter(([name])=>!categories||categories.includes(name));},[library,role]);
  const visible=useMemo(()=>{const term=query.trim().toLowerCase();return permittedDocuments.filter(document=>(category==='All documents'||document.category===category)&&(!term||`${document.title} ${document.subtitle} ${document.summary} ${document.sourcePath}`.toLowerCase().includes(term)));},[category,permittedDocuments,query]);
  const selected=visible.find(document=>document.id===selectedId)||visible[0];
  useEffect(()=>{if(category!=='All documents'&&!permittedCategories.some(([name])=>name===category))setCategory('All documents');},[category,permittedCategories]);
  ...
  return <div className="repository-library"><div className="docs-role-notice"><span>Role-based document library</span><p>Showing <strong>{permittedDocuments.length}</strong> documents approved for your <strong>{roleLabels[role]}</strong> account. Restricted internal collections stay hidden.</p></div><div className="repository-controls"><label><span>Search {permittedDocuments.length} available documents</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search titles, summaries, and source paths"/></label><label><span>Collection</span><select value={category} onChange={event=>setCategory(event.target.value)}><option>All documents</option>{permittedCategories.map(([name,count])=><option key={name} value={name}>{name} ({count})</option>)}</select></label></div><div className="docs-layout repository-layout"><aside className="docs-index repository-index"><p>{visible.length} of {permittedDocuments.length} available</p>{visible.map(document=><button key={document.id} className={selected?.id===document.id?'active':''} onClick={()=>setSelectedId(document.id)}><span>{document.category}</span><strong>{document.title}</strong><small>{document.readMinutes} min · {document.words.toLocaleString()} words</small></button>)}</aside><article className="docs-article repository-article">{selected&&<><header><span>{selected.category}</span><h2>{selected.title}</h2><p>{selected.summary}</p><small>{selected.readMinutes} minute read · {selected.headings} sections · Source: {selected.sourcePath}</small><nav className="repository-downloads" aria-label="Document downloads"><a href={selected.markdownUrl} download>Markdown</a><a href={selected.docxUrl} download>Word</a><a href={selected.pdfUrl} target="_blank" rel="noreferrer">PDF</a></nav></header>{loadingDocument?<div className="docs-empty">Loading document…</div>:error?<div className="docs-empty" role="alert">{error}</div>:<MarkdownDocument markdown={markdown}/>}</>}</article></div></div>;
}
