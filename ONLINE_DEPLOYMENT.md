...
The repository includes a Render Blueprint for a persistent release environment. It creates two web services and a private PostgreSQL database from `main`:
...
- `servicepro-postgres`: the authoritative PostgreSQL datastore

## Start the deployment

1. Open the [ServicePro Render Blueprint](https://render.com/deploy?repo=https%3A%2F%2Fgithub.com%2Fckcgoodnews-oss%2Fservicepro-real-runtime).
2. Sign in to Render with GitHub and authorize access to `ckcgoodnews-oss/servicepro-real-runtime` if prompted.
3. Supply the prompted HTTPS values for `CORS_ALLOWED_ORIGINS` and `NEXT_PUBLIC_API_BASE_URL`, review the two Starter web services and PostgreSQL database, and choose **Deploy Blueprint**.
4. Wait for both services to report **Live** and for the pre-deploy migration command to complete.
...
6. Provision a tenant owner through the approved company-provisioning process, then open `https://servicepro-web-alpha-ckcgoodnews.onrender.com/login` and sign in with that account.
7. Open `/system-status` from the workspace sidebar and confirm every readiness check passes.

The Blueprint deploys `main`. Each push runs the full GitHub CI workflow, and Render deploys the commit only after those checks pass. See `LIVE_TESTING.md` for the local and live test loop.

Before an online deploy, run `npm run deploy:validate:online` and the local release gates from the repository root. After provisioning an owner, use the authenticated deployed-app smoke test to verify health, readiness, and login against PostgreSQL.
...
Enter the provisioned company's tenant key and choose expected store `postgres`; the smoke rejects temporary storage and cross-tenant authentication.
...
Set `SMOKE_TIMEOUT_MS=90000` when the environment needs a longer deployment warm-up window.
...
The online environment uses PostgreSQL and never enables the local demonstration credentials automatically. Provision each company through the approved workflow, rotate all bootstrap credentials, configure custom domains, and merge releases through `main` only with passing CI. See `SUPABASE_DEPLOYMENT.md` for the dedicated-project-per-company option and complete migration bundle.
