...
Sprint 727 continuation:

```powershell
npm run test:sprint727
npm run migrations:check
npm run web:typecheck
npm run web:build
npm run web:build:pages
npm test
git add -- apps/api/src/repositories/serviceMarketplaceRepository.js apps/api/src/routes/serviceMarketplace.js apps/api/src/router.js apps/api/src/store/jsonStoreAdapter.js apps/web/src/app/(workspace)/marketplace/page.tsx apps/web/src/app/globals.css apps/web/src/app/marketplace.css apps/web/src/components/AppShell.tsx apps/web/src/components/MarketplaceWorkspace.tsx packages/database/postgres/727_marketplace_experience.sql tests/sprint727-marketplace-experience.test.js SPRINT727_REQUIRED_WIRING.md PHASE46_RELEASE_NOTES.md GIT_COMMANDS_PHASE46.md package.json
git commit -m "Sprint 727: add multi-industry marketplace"
git push origin codex/sprint-716-frontend-foundation
```
